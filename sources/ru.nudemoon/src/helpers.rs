...
pub fn browse_url(page: i32, sort_index: usize, genres: &[String]) -> String {
	...
	if genres.is_empty() {
		...
		// Verified via mitmproxy: the site serves tag pages at
		// /tags/{slug}_&{sort}&rowstart={n} — tags joined by `_`,
		// trailing `_` before sort params.
		format!(
...
mod tests {
	...
	fn browse_url_combines_genres_and_sort() {
		// Tags joined by +, sort in query via ?
